...
class TopFrame(wxfb_out.TopFrame):
    def __init__(self):
        ...
        self.Maximize()
        self.Center()
        self.load_profile('profiles/leaded-profile.prfl')
    ...
    def load_profile(self, fname):
        ...
        lines = [L.strip().upper().split(',') for L in open(fname,'r').readlines() if L.strip() and (L.strip()[0] != '#')]
    ...
    def on_open_profile(self, event):
        ...
        if profile_filename:
            return self.load_profile(profile_filename)
